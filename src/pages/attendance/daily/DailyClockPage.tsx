...
export default function DailyClockPage() {
  ...
  const getTodayDailyAttendanceInfo = useCallback(async () => {
    try {
  ...
  const handleClock = async (type: "IN" | "OUT") => {
    ...
    try {
      ...
      const response =
        type === "IN"
          ? await api.post(
              `/attendance/clock-in/${currentUser?.username}`,
              formData,
              {
                headers: {
          ...
          : await api.put(
              `/attendance/clock-out/${selectedAttendance?.id}`,
              formData,
              {
                headers: {
                  "Content-Type": "multipart/form-data",
  ...
  useEffect(() => {
    if (position?.lat && position.lng) {
      getTodayDailyAttendanceInfo();
    }
  }, [getTodayDailyAttendanceInfo, position?.lat, position?.lng]);
              ...
              {!selectedAttendance ? (
                ...
                <div className="space-y-0.5 text-[11px]">
                  {selectedAttendance.clockIn &&
                    (() => {
                      const [dateIn, timeIn] =
                        selectedAttendance.clockIn.split("T");
                      return (
                        <p>
                  ...
                  {selectedAttendance.clockOut &&
                    (() => {
                      const [dateOut, timeOut] =
                        selectedAttendance.clockOut.split("T");
                      return (
                        <p>
            ...
            <AlertDialogFooter>
              <AlertDialogAction
                onClick={async () => {
                  ...
                  await getTodayDailyAttendanceInfo();
                }}
              >
                Ok
              </AlertDialogAction>
